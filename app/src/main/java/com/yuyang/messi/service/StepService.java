...
public class StepService extends Service implements SensorEventListener {
    ...
    private String channelId = "step";//过长会引发震动，我也很无奈
    /**
     * 默认为30秒进行一次存储
     */
    ...
    private void initTodayData() {
        ...
        //获取当天的数据，用于展示
        StepBean stepBean = stepDao.getByDate(CURRENT_DATE);
        if (stepBean == null) {
            CURRENT_STEP = 0;
        } else {
            CURRENT_STEP = stepBean.getStep();
        }
        if (mStepCount != null) {
...
//                        save();
    ...
    private void newDay() {
        String time = "00:00";
        if (time.equals(new SimpleDateFormat("HH:mm").format(new Date())) || !CURRENT_DATE.equals(sdf.format(new Date()))) {
            initTodayData();
        }
    }
    ...
    private void updateNotification() {
        //设置点击跳转
        Intent hangIntent = new Intent(this, StartActivity.class);
