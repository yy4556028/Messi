...
public class InterviewMediaActivity extends AppBaseActivity {

    @Override
    protected int getLayoutId() {
        return R.layout.activity_interview_common;
    }
    ...
    private void initView() {
        ...
        headerLayout.showTitle("音视频");

        TextView tvDetail = findViewById(R.id.tvDetail);
        tvDetail.append(StringDealUtil.highlightKeyword(Color.RED,
                "VCL：VideoCodingLayer 视频码流层：高效展示视频内容，压缩编码后的视频数据序列\n" +
                        "NAL：NetworkAbstractionLayer 网络提取层：以网络要求的方式对数据打包传送。无论本地和网络，都要通过NAL\n"+
                        "NAL装在一个个片(slice),片里面装在一个个宏块(像素块)\n\n"
                ));

        tvDetail.append(StringDealUtil.highlightKeyword(Color.BLUE,
                "I帧：关键帧,P帧,B帧\n" +
                        "H264也叫 MPEG-4 AVC，是同一个视频编码方案的不同叫法(两个公司一起研发)\n" +
                        "：\n\n",
