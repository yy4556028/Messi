...
public class DateTimePicker extends WheelPicker {
    ...
    protected View makeCenterView() {
        ...
        if (mode != MONTH_DAY) {
            ...
            yearView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                ...
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    ...
                    //需要根据年份及月份动态计算天数
                    days.clear();
                    int maxDays = DateUtil.getMonthDayCount(stringToYearMonthDay(item), stringToYearMonthDay(months.get(selectedMonthIndex)));
                    for (int i = 1; i <= maxDays; i++) {
                        days.add(DateUtil.fillZero(i));
                    }
        ...
        monthView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                if (mode != YEAR_MONTH) {
                    ...
                    days.clear();
                    int maxDays = DateUtil.getMonthDayCount(stringToYearMonthDay(years.get(selectedYearIndex)), stringToYearMonthDay(item));
                    for (int i = 1; i <= maxDays; i++) {
                        days.add(DateUtil.fillZero(i));
                    }
                    if (selectedDayIndex >= maxDays) {
        ...
        for (int i = 0; i < 60; i++) {
            minutes.add(DateUtil.fillZero(i));
        }
    ...
    public DateTimePicker(Activity activity, @Mode int mode) {
        ...
        for (int i = 1; i <= 12; i++) {
            months.add(DateUtil.fillZero(i));
        }
        for (int i = 1; i <= 31; i++) {
            days.add(DateUtil.fillZero(i));
        }
        selectedHour = DateUtil.fillZero(Calendar.getInstance().get(Calendar.HOUR_OF_DAY));
